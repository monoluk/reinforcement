...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if not legalActions:
          ...
          for action in legalActions:
            qVlues.append(self.getQValue(state,action))
        #return the highest value among the values
        return max(qVlues)
    ...
    def computeActionFromQValues(self, state):
        ...
        bestIndices = [index for index in range(len(legalActions)) if self.getQValue(state, legalActions[index]) == bestValue]
    ...
    def update(self, state, action, nextState, reward):
        ...
        if not nextActions:
          ...
          #get the qStart value from the next state
          nextMax = max(nextQvalues)
        #create sample from the next state
        sample = reward + self.discount*nextMax
        #apply the MDP formula to update qValue
